...
hotel_reviews %>% group_by(Hotel_Address) %>% summarise(name=first(Hotel_Name), count=n()) %>% 
  arrange(desc(count)) %>% dplyr::select(name, count) %>% head() #select() may get masked by another package therefore specify dplyr
...
#hotels with low scores
hotel_agg %>% filter(score<7)
```
...
Performe variance test at places where lmod0 showes deficiency, both tests produce greater p values than before
```{r}
...
```{r}
lmod_x1 = lm(score~review_t1+review_n+review_p+review_m1+reviewer_exp, hotel_agg1, subset = (cook < 0.3))
summary(lmod_x1)
```
...
for(i in 1:k){
  ...
  model=lm(score~review_t1+review_n+review_p+review_m1+reviewer_exp,train,subset = (cook < 0.3))
...
```{r}
<<<<<<< HEAD
# class example of K fold average rmse
cv.glm(data=hotel_agg1, glm(score ~ review_t1 + review_n + review_p + review_m1 + reviewer_exp, data=hotel_agg1), K=5)$delta[1]
=======
# class example of K fold CV
mse_avg_1=cv.glm(data=hotel_agg1, glm(score ~ review_t1 + review_n + review_p + review_m1 + reviewer_exp, data=hotel_agg1), K=5)$delta[1]
mse_avg_1^(1/2)
>>>>>>> d6e93dbb3cd499447cc1fedea0879c9c6cdb553f
```
...
lmod_x1_noexp = lm(score~review_t1+review_n+review_p+review_m1, hotel_agg1, subset = (cook < 0.3))
...
for(i in 1:k){
  test=hotel_agg1[fold==i,] # test set is the ith group for the ith iteration
  train=hotel_agg1[fold!=i,]# training set is all the other groups
  model1=lm(score~review_t1+review_n+review_p+review_m1,train,subset = (cook < 0.3))
...
<<<<<<< HEAD
# class example of K fold average rmse
cv.glm(data=hotel_agg1, glm(score ~ review_t1 + review_n + review_p + review_m1, data=hotel_agg1), K=5)$delta[1]
=======
# class example of K fold CV
mse_avg_2=cv.glm(data=hotel_agg1, glm(score ~ review_t1 + review_n + review_p + review_m1, data=hotel_agg1), K=5)$delta[1]
