...
One hotel with the lowest score is removed. This hotel has much fewer number of reviews in total than the others, which indicates this may be a new hotel.
...
#var.test(residuals(lmod_x)[hotel_agg1$review_p>28], residuals(lmod_x)[hotel_agg1$review_p<28])
...
Model with transformed predictors passes DW test at $\alpha=5\%$.

```{r}
#randomize dataset for DW test as they were ordered by address
hotel_agg1_r <- hotel_agg1[sample(nrow(hotel_agg1)),]
dwtest(lm(score ~ review_t1 + review_n + review_p + review_m1 + reviewer_exp, hotel_agg1_r))
```

### multicolliearity - Variance Inflation Factor

The max Variance Inflation Factor is smaller than 10 and the average of VIF is less than 3. We think the predictors have no multicolliearity.
...
Bonferroni-corrected criteria here cannot rule out any outlier.

```{r}
nrow(hotel_agg1)
stud = rstudent(lmod_x)
jackres = stud*(1485/(1486-stud^2))^0.5
head(jackres[order(abs(stud), decreasing = T)], 20)
qt(.025/1492, 1485)
```
...
```{r}
cook = cooks.distance(lmod_x)
halfnorm(cook, 1, ylab = "Cook's distance")
```

The removed hotel here has much fewer reviews in total and a high score.
```{r}
hotel_agg1[1411,c(1:8)] %>% knitr::kable()

hotel_agg2 <- hotel_agg1[-1411,] # subset hotel_agg1 to remove the outlier
```
...
Targeted model is the one with transformed predictors (lmod_x1) having outliers removed.

### forward search

```{r}

out.null = lm(score ~ 1, hotel_agg2)
out.full = formula(lmod_x1)

out_f = regsubsets(out.full, data = hotel_agg2, method = 'forward', nvmax = 5)
...
```{r}
plot(c(1, 2, 3, 4, 5), bic, xlab="predictors")
```
...
## 5 Fold Cross Validation for lmod_x1
methond 1
```{r}
set.seed(1)
k=5
fold=sample(1:k,nrow(hotel_agg2),replace=TRUE)

kfold.rmse=1:k # we will have 5 RMSEs to fill this with later
for(i in 1:k){
  test=hotel_agg2[fold==i,] # test set is the ith group for the ith iteration
  train=hotel_agg2[fold!=i,]# training set is all the other groups
...
mse_avg_1=cv.glm(data=hotel_agg2, glm(score ~ review_t1 + review_n + review_p + review_m1 + reviewer_exp, data=hotel_agg2), K=5)$delta[1]
mse_avg_1^(1/2)
```

## 5 Fold Cross Validation for lmod_x1_noexp (reduced model using early stopping)
```{r}
#If we ddecide to get rid of variable exp based on forward selection
lmod_x1_noexp = lm(score~review_t1+review_n+review_p+review_m1, hotel_agg2)
summary(lmod_x1_noexp)
```
...
```{r}
# class example of K fold CV
mse_avg_2=cv.glm(data=hotel_agg2, glm(score ~ review_t1 + review_n + review_p + review_m1, data=hotel_agg2), K=5)$delta[1]
